									...
									<div class="booking_info">
										...
										<div class="col-sm-5 brder_left">
											...
											?>
												...
												ZapBooking Reference Number : {{$set->ref_prefix}}-{{@$fetchedData->id}}</p>
											</div>
										</div>
										<div class="col-sm-5">
											<div class="booking_thanks">
												<p>Thank-you for choosing ZapBooking. Your Booking is <span>confirmed</span> and the E-Ticket has been mailed to you. Please carry a printout of your E-Ticket along with Passport (If required) to the Airline Check-in Counter.</p>
											</div>
										</div>
...
jQuery(document).ready(function($){
	$(".resend_mail").on("click", function(e){
		...
		$('.mail-msg').hide();
		$.ajax({
			url: "{{ URL::to('/agent/emailticket') }}/"+tcid,
			dataType: 'json',
			type: 'GET',
			data: {email:mail},
	...
	$(".resend_phone").on("click", function(e){
		...
		var mail = $('#phone').val();
			...
			$('.mail-msg').hide();
				 $.ajax({
					url: "{{ URL::to('/agent/phoneticket') }}/"+tcid,
					dataType: 'json',
					type: 'GET',
					data: {phone:mail},
					success: function( data ){
