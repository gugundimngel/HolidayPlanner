...
<ul>
	...
	$markupd =0;
	$submark =0;
	$markupamt = \App\Markup::where('flight_code', $res->Segments[0][0]->Airline->AirlineCode)->where('flight_type', $is_international)->where('user_type', 'b2b')->first(); 
	if($markupamt){
	...
	$remarkupd =0;
	$sremarkupd =0;
	$remarkupamt = \App\Markup::where('flight_code', $resarrive->Segments[0][0]->Airline->AirlineCode)->where('flight_type', $is_international)->where('user_type', 'b2b')->first(); 
	if($remarkupamt){
...
<div id="bookingCounter" class="">
	...
	<div class="msessionpop" style="display: none;">
			<div class="searchpopinner searchpopinner1">
				...
				<a href='{{URL::to("/agent/flight/index")}}'>go to homepage.</a>
				<div class="clearfix"></div>
			</div> 
		</div>
