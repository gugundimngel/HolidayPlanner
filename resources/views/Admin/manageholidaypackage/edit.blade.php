			...
			<div class="row">
				...
				<div class="col-md-12">
					<div class="card card-primary">
					  ...
					  <!-- /.card-header -->  
					  <!-- form start -->
					  {{ Form::open(array('url' => 'admin/holidaypackage/edit', 'name'=>"edit-package", 'autocomplete'=>'off', "enctype"=>"multipart/form-data")) }}
								  ...
								  <div class="tab-pane fade" id="vert-tabs-price" role="tabpanel" aria-labelledby="vert-tabs-price-tab">
										...
										<table class="table table-hover text-nowrap">
											...
											@foreach(\App\PackagePrice::where('package_id', $fetchedData->id)->get() as $pricelist)
												<?php
												$pricedata = 'departure_date='.$pricelist->departure_date.'&seats='.$pricelist->no_of_seats.'&twin='.$pricelist->twin.'&triple='.$pricelist->triple.'&single='.$pricelist->single.'&child_with_bed='.$pricelist->child_with_bed.'&child_without_bedbelow12='.$pricelist->child_without_bedbelow12.'&child_without_bedbelow26='.$pricelist->child_without_bedbelow26.'&dis_type='.$pricelist->dis_type.'&adult_flight='.$pricelist->adult_flight.'&booking_amt='.$pricelist->booking_amt.'&child_flight='.$pricelist->child_flight.'&infant_flight='.$pricelist->infant_flight.'&infant='.$pricelist->infant.'&price_type='.$pricelist->price_type.'&dis_type='.$pricelist->dis_type.'&balance_rec='.$pricelist->bal_rec_day.'&priceid='.$pricelist->id.'&flightname='.$pricelist->flightname.'&flight_type='.$pricelist->flight_type.'&dep_city='.$pricelist->dep_city.'&arv_city='.$pricelist->arv_city.'&dep_time='.$pricelist->dep_time.'&arv_time='.$pricelist->arv_time;
												?>
												<tr id="{{$ipsb}}">
													<td>{{$pricelist->departure_date}}</td>
													<td>{{$pricelist->no_of_seats}}</td>
													<td><a href="javascript:;" class="remove_pricerow"><i class="fa fa-trash"></i></a>/<a  href="javascript:;" class="edit_pricerow"><i class="fa fa-edit"></i></a><input type="hidden" class="priceblo" name="dep_price[]" value="{{$pricedata}}"></td>
												</tr>
												<?php $ipsb++; ?>
											@endforeach
											</tbody>
								  ...
								  <div class="tab-pane fade" id="vert-tabs-exclusions" role="tabpanel" aria-labelledby="vert-tabs-exclusions-tab">
								   ...
								   <div class="tab-pane fade" id="vert-tabs-visa" role="tabpanel" aria-labelledby="vert-tabs-visa-tab">
										<h4>Visa</h4>
										 <div class="form-group row">
											<div class="col-sm-12">
												<textarea name="visa_overview" data-valid ="" class="textarea" placeholder="" style="width: 100%; height: 200px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;">{{$fetchedData->visa_overview}}</textarea>
...
jQuery(document).ready(function($){
	...
	$(document).delegate('.edit_pricerow', 'click', function(){
		...
		$('input[name="priceid"]').val(obj.priceid);
			...
			if($.inArray("required", splitDataValidation) !== -1) //for required
				...
				if(i > 0)
				{
					...
					//$('.pricedata tr#'+vs+' .pricedatainfo').html('<a href="javascript:;" class="remove_pricerow"><i class="fa fa-trash"></i></a>/<a href="javascript:;" class="edit_pricerow"><i class="fa fa-edit"></i></a><input type="hidden" class="priceblo" name="dep_price[]" value="'+pricedata+'">');
					 $('.pricedata tr#'+vs).html('<td>'+obj.departure_date+'</td><td>'+obj.seats+'</td><td><a href="javascript:;" class="remove_pricerow"><i class="fa fa-trash"></i></a>/<a href="javascript:;" class="edit_pricerow"><i class="fa fa-edit"></i></a><input type="hidden" class="priceblo" name="dep_price[]" value="'+pricedata+'"></td>'); 
							...
							<div class="row">
								...
								<div class="col-sm-4">
									<div class="form-group row">
										...
										<input type="hidden" value="" name="priceid">
									</div>
								</div>
							</div>
						</div>
