$(function() {
   ...
   if(ext=="pdf"){
	...
	$(document).delegate('.selectimage','click', function(e){
		...
		var mtype = $('#mtype').val();	
		$('.'+mtype).html('<input type="hidden" id="'+mtype+'" name="'+mtype+'" value="'+id+'"><img width="100" height="100" src="'+image+'"><a href="javascript:;" class="removepackimage" rdatatype="'+mtype+'" id="'+id+'"><i class="fa fa-times"></i></a>');
...
$("#loader").show();
    $.ajax({
        ...
        success: function(response){
			...
			var obj = response; 
			if(obj.success){
				$('.gallery_list_data ul').append('<li id="l_'+obj.id+'"><img class="selectimage" dataimage="'+media_image_url+'/'+obj.imagedata+'" d="s_'+obj.id+'" dataid="'+obj.id+'" src="'+media_image_url+'/'+obj.imagedata+'"><a href="javascript:;" class="removeimage" removeid="sho_'+obj.id+'" id="'+obj.id+'"><i class="fa fa-times"></i></a></li>');
			}else{
				$('.show_custom_msg').html('<div class="alert alert-danger" role="alert">There is something problem. Please try again</div>');
			}
        }
