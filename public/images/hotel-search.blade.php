...
.myBar {width: 1%; height: 2px; background-color: #757588;}
 ...
 function fetch_data(arrCount)
 {
		...
		success:function(data)
		{
			...
			if (arrCount < elementArray){
				...
				if(obj.success){
					//minpricearray.push(obj.filtersMeta.hotelprice.min); 
					...
					$.each(faciltiesarray, function (indexs, values) {
						...
						$('#myULairfac').append('<li style="" class="'+indexs+'"><label class="label-container checkbox-default">'+values+'<input name="property_type" class="chboxAirline hotelfacilties" type="checkbox" value="'+values+'"><span class="checkmark"></span></label></li>');
						}
					});
