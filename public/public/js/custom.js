...
//Delete Function Start
	...
	function deletenewAction( id, table ) {
		var conf = confirm('Are you sure, you would like to delete this record');
		if(conf){	 
			if(id == '') {
				alert('Please select ID to delete the record.');
				return false;	
				...
				$.ajax({
					type:'post',
					headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
					url:site_url+'/admin/delete_newaction',
					data:{'id': id, 'table' : table},
					success:function(resp) {
						var obj = $.parseJSON(resp);
						if(obj.status == 1) {
  ...
  $('.custom-error-msg').html(html);
   setTimeout(function(){
	...
	//$(document).delegate('.change-status','click', function(){
		...
		$('.change-new-status').on('switchChange.bootstrapSwitch', function (event, state) {
			var id = $.trim($(this).attr('data-id'));
			var current_status = $.trim($(this).attr('data-status'));
			var table = $.trim($(this).attr('data-table'));
			var col = $.trim($(this).attr('data-col'));
			...
			if(id != "" && current_status != "" && table != ""){
				updatenewStatus(id, current_status, table, col);
			}
		});
	...
	function updatenewStatus( id, current_status, table,col ) {
		$(".server-error").html(''); //remove server error.	
		$(".custom-error-msg").html(''); //remove custom error.
		$.ajax({
			type:'post',
			headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
			url:site_url+'/admin/update_new_action',
			data:{'id': id, 'current_status' : current_status, 'table': table, 'colname':col},
			success:function(resp) {
				var obj = $.parseJSON(resp);
				if(obj.status == 1) {
...
function utoa(data) {
  return btoa(unescape(encodeURIComponent(data)));
	...
	$(document).delegate('.edit_itinerary','click', function(){
		...
		$('#itinerary_modal .note-editable').html(' ');
		$('#foodtype').val([]).trigger('change');
		$('#itinerary_detail').val(atou(itinerarydata[val]['detail']));
		$('#itinerary_modal .note-editable').html(atou(itinerarydata[val]['detail']));
		if(itinerarydata[val]['image'] != ''){
	...
	$('#save_metatag').on('click', function(){		
		...
		var canonicaltag = $('#canonicaltag').val(); 		
		...
		if(metatitle != ''){
			metatagdata[itag] = {
					...
					"canonicaltag":btoa(canonicaltag),
				}	
			$('#hide_metatag').hide();
			var html = '<tr id="metatag_'+itag+'"><td>'+metatitle+'</td><td>'+metakeyword+'</td><td>'+metadescription+'</td><td>'+canonicaltag+'</td><td><a class="remove_metatag" id="'+i+'" href="javascript:;"><i class="fa fa-trash"></i></a> / <a href="javascript:;" class="edit_metatag" id="'+itag+'"><i class="fa fa-edit"></i></a><input type="hidden" name="all_meta_title[]" value="'+metatitle+'"><input type="hidden" name="all_meta_keyword[]" value="'+metakeyword+'"><input type="hidden" name="canonicaltag[]" value="'+canonicaltag+'"><textarea style="display:none;" type="hidden" name="all_meta_desc[]" value="">'+metadescription+'</textarea></td></tr>';
	...
	$(document).delegate('.edit_metatag','click', function(){
		...
		$('#canonicaltag').val(atob(metatagdata[val]['canonicaltag']));
	...
	$(document).delegate('#update_metatag','click', function(){
		...
		var canonicaltag = $('#canonicaltag').val(); 
			...
			if(metatitle != ''){
				...
				$('#hide_metatag').hide();
			var html = '<td>'+metatitle+'</td><td>'+metakeyword+'</td><td>'+metadescription+'</td><td>'+canonicaltag+'</td><td><a class="remove_metatag" id="'+i+'" href="javascript:;"><i class="fa fa-trash"></i></a> / <a href="javascript:;" class="edit_metatag" id="'+mtval+'"><i class="fa fa-edit"></i></a><input type="hidden" name="all_meta_title[]" value="'+metatitle+'"><input type="hidden" name="all_meta_keyword[]" value="'+metakeyword+'"><input type="hidden" name="canonicaltag[]" value="'+canonicaltag+'"><textarea style="display:none;" type="hidden" name="all_meta_desc[]" value="">'+metadescription+'</textarea></td>';
