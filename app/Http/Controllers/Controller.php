...
public static function getnationalityRe(){
	     $country = Country::get();
		return $country;
	}
	
	public static function callender() {
		?>
...
{{ 
  ...
  <table class="calendar-table table table-condensed table-tight">
          ...
          <table style="white-space: nowrap; width: 100%">
            <tr>
              <td style="text-align: left;">
                ...
                <button class="js-cal-option {{: first.toDateInt() <= today.toDateInt() && today.toDateInt() <= last.toDateInt() ? 'active':'' }}" data-date="{{: today.toISOString()}}" data-mode="month">{{: todayname }}</button>
              </td>
              <td>
                <span class="btn-group btn-group-lg">
                  {{ if (mode !== 'day') { }}
                    {{ if (mode === 'month') { }}<button class="js-cal-option btn-link" data-mode="year">{{: months[month] }}</button>{{ } }}
                    {{ if (mode ==='week') { }}
                      <button class=" btn-link disabled">{{: shortMonths[first.getMonth()] }} {{: first.getDate() }} - {{: shortMonths[last.getMonth()] }} {{: last.getDate() }}</button>
                    {{ } }}
                    <button class="js-cal-years  btn-link">{{: year}}</button> 
                  {{ } else { }}
                    <button class=" btn-link disabled">{{: date.toDateString() }}</button> 
                  {{ } }}
                </span>
              </td>
	...
	public static function Country_($id){
	if($id){
...
curl_setopt_array($curl, array(
  ...
  CURLOPT_HTTPHEADER => array(
    ...
    "postman-token: dde80a72-0ec1-72ed-938c-c1c7540e981b"
  ),
));
...
curl_close($ch);
	...
	public function GetCalculatePlan($data){
		
			$searchdata = Session::get('allrequest');
			$srch = $searchdata['srch'];
			$explodesearc = explode('|', $srch);
			$datedeparture = date('Y-m-d', strtotime($explodesearc[2]));
			if(isset($explodesearc[3])){
				$todate = date('d-M-Y', strtotime($explodesearc[3]));
			}else{
				$todate = date('d-M-Y', strtotime($datedeparture. '+ 5 days'));
			}
		...
		$urls = "https://api.bagicpp.bajajallianz.com/BjazTravelWebServices/calculatepremium";
			$datas = '
			{
    ...
    "travelpremiumin": {
        ...
        "pdateofbirth": "01-Jun-1981",
        "pfromdate": "'.date('d-M-Y', strtotime($datedeparture)).'",
    ...
    $datas .='"familyparamlist": [';
	$datasa = ''; 
	for($i=0;$i<count($data['adultfirstname']); $i++){
		if($data['adulttitle'][$i] == 'Mr'){
			$sex = 'M';
		}else{
			$sex = 'F';
		}
        $datasa .='{
            "pvassignee": "Legal Air",
            "pvpartnerid": "",
            "pvrelation": "SELF",
            "pvname": "'.$data['adultfirstname'][$i].' '.$data['adultlastname'][$i].'",
            "pvpassportno": "",
            "pvage": "29",
            "pvsex": "'.$sex.'",
            "pvdob": "01-Jun-1981"
	...
	for($ic=0;$ic<count($data['childfirstname']); $ic++){
		if($data['childtitle'][$ic] == 'Master'){
			$sex = 'M';
		}else{
			$sex = 'F';
		}
		$chexp = explode('/',$data['childdob'][$ic]);
		$dataachild = $chexp[0].'-'.$chexp[1].'-'.$chexp[2];
		$childdob = date('d-M-Y', strtotime($dataachild));
		$diff = (date('Y') - date('Y',strtotime($childdob)));
        $datasa .='{
            "pvassignee": "Legal Air",
            "pvpartnerid": "",
            "pvrelation": "SELF",
            "pvname": "'.$data['childfirstname'][$ic].' '.$data['childfirstname'][$ic].'",
            "pvpassportno": "",
            "pvage": "'.$diff.'",
            "pvsex": "'.$sex.'",
            "pvdob": "'.$childdob.'"
	...
	for($ii=0;$ii<count($data['infantfirstname']); $ii++){
		if($data['infanttitle'][$ii] == 'Master'){
			$sex = 'M';
		}else{
			$sex = 'F';
		}
		$inexp = explode('/',$data['infantdob'][$ii]);
		$dataainfant = $inexp[0].'-'.$inexp[1].'-'.$inexp[2];
		$infantdob = date('d-M-Y', strtotime($dataainfant));
		$diff = (date('Y') - date('Y',strtotime($infantdob)));
        $datasa .='{
            "pvassignee": "Legal Air",
            "pvpartnerid": "",
            "pvrelation": "SELF",
            "pvname": "'.$data['infantfirstname'][$ii].' '.$data['infantfirstname'][$ii].'",
            "pvpassportno": "",
            "pvage": "'.$diff.'",
            "pvsex": "'.$sex.'",
            "pvdob": "'.$infantdob.'"
...
curl_setopt_array($curl, array(
  CURLOPT_URL => "https://auth.bagicpp.bajajallianz.com/auth/realms/Bagic/protocol/openid-connect/token",
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_ENCODING => "",
  CURLOPT_MAXREDIRS => 10,
  CURLOPT_TIMEOUT => 30,
  ...
  CURLOPT_POSTFIELDS => "client_id=B2BService_Client&client_secret=372c2e66-f5bc-4c53-a85a-45f8f69da332&username=adminholy1@insurance.com&password=Bagic123&grant_type=password",
  CURLOPT_HTTPHEADER => array(
    "accept: application/json",
    "cache-control: no-cache",
    "content-type: application/x-www-form-urlencoded",
    "postman-token: 25b7c0fd-aedb-5bdb-44af-5341773bc12b"
  ),
));
...
public function GetPolicyImage($token,$policy){
	$curl = curl_init();

curl_setopt_array($curl, array(
  CURLOPT_URL => "https://api.bagicpp.bajajallianz.com/bjazDownload/travel/downloadPdf/".$policy,
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_ENCODING => "",
  CURLOPT_MAXREDIRS => 10,
  CURLOPT_TIMEOUT => 30,
...
if ($err) {
  ...
  return $response;
	...
	public function GetissuePolicy($data){
		$searchdata = Session::get('allrequest');
			$srch = $searchdata['srch'];
			$explodesearc = explode('|', $srch);
			$datedeparture = date('Y-m-d', strtotime($explodesearc[2]));
		...
		$urls = "https://api.bagicpp.bajajallianz.com/BjazTravelWebServices/issuepolicy";
		$datas = '{
  "userid": "adminholy1@insurance.com",
  "password": "Bagic123",
  "bjaztravelissue": {
    "passigneename": "'.$data['adultfirstname'][0].'",
    "pfulltermpremium": "0",
    "telephone3": "'.$data['phone'].'",
    "ptotalpremium": "0",
    "telephone2": "",
    "nationalid": "INDIA",
    "userid": "adminholy1@insurance.com",
    ...
    "maritalstatus": "",
    "partnertype": "P",
    "ptermstartdate": "'.date('d-M-Y', strtotime($datedeparture)).'",
    "regnumber": "0",
    "plocationcode": "",
    "pempno": "0",
    "policyno": "",
	...
	public function GetAuthenticate()
    {
		...
		if($flghtapi->flight_api_type == 1){
			...
			$data = array(
				...
				'UserName' => 'CHACHA.1',
				'Password' => 'Chacha@4321',
				'EndUserIp' => '106.213.12.137',
			);
			$jsonresponse = json_encode($data);
		}
		
	//if($flghtapi->flight_api_type == 1){
		/* if(\Cookie::get('mylivetoken') != ''){
			$token = \Cookie::get('mylivetoken');
		}else{
			$authjson = $this->postcurlRequest($url,$jsonresponse);
			$auth = json_decode($authjson);
			
			\Cookie::queue(\Cookie::make('mylivetoken', $auth->TokenId, strtotime( '+1 days' )));
...
// 			dd($myauth);
	...
	public function GetAgentAuthenticate()
    {
		 $flghtapi = WebsiteSetting::where('id', '!=', '')->first();
		...
		$authjson = $this->postcurlRequest($url,$jsonresponse);
			$myauth = json_decode($authjson);  
		return $myauth;
    }
	
	public static function GetFlightTimeduration($time){
		
		$alltime =0;
		foreach($time as $k => $l){
			
			/* if(@$l->Duration == 0){
				$alltime += @$l->AccumulatedDuration;
			}else{
				//$alltime += @$l->Duration + @$l->GroundTime;
			} */
			if(!empty(@$l->AccumulatedDuration)){
				$alltime = @$l->AccumulatedDuration;
			}else{
				$alltime = @$l->Duration;
			}
		}
			$hours = floor($alltime / 60);
			$minutes = ($alltime % 60);
			$newhours = sprintf("%02d", $hours);
		$newminutes = sprintf("%02d", $minutes);
		return $newhours.'h'.' '.$newminutes.'m';
			
	}
	
	public static function GetFetilFlightTimeduration($time){
		
		$alltime =0;
		/* foreach($time as $l){ */
			
			if(@$time->Duration == 0){
				$alltime += @$time->AccumulatedDuration;
			}else{
	...
	public static function GetTimeduration($firsttime, $secondtime)
    {
		$date1 = strtotime($firsttime);  
		$date2 = strtotime($secondtime);  
		...
		public function sendPasswordEmail($email, $token)
		{
		...
		$link = \URL::to('/') . '/password/reset/' . $token . '?email=' . urlencode($user->email);
			...
			try {
				...
				$replaceav = array('{customer_name}','{link}');
				$replace_withav = array(@$user->first_name.' '.@$user->last_name, $link);			
				$emailtemplate	= 	DB::table('email_templates')->where('alias', 'new-password')->first();
				
				
				$subContent 	= 	$emailtemplate->subject;
				
				$issuccess = $this->send_email_template($replaceav, $replace_withav, 'new-password', $user->email,$subContent,'info@crm.travelsdata.com'); 
				if($issuccess){
					return true;
				}else{
					return false;
	...
	public function sendResetEmail($email, $token)
		...
		$link = \URL::to('/') . '/password/reset/' . $token . '?email=' . urlencode($user->email);
			...
			try {
				...
				$replaceav = array('{logo}','{customer_name}','{link}','{company_name}','{company_email}', '{expire_date}');
				$replace_withav = array(\URL::to('/').'/public/img/profile_imgs/'.@$set->logo, @$user->first_name.' '.@$user->last_name, $link, $set->company_name, $set->primary_email, date('M d, Y',strtotime('+24 hours')));			
				$emailtemplate	= 	DB::table('email_templates')->where('alias', 'reset-password')->first();
				
				$replacesub = array('{company_name}');					
			$replace_with_sub = array($set->company_name.' Account');
				$subContent 	= 	$emailtemplate->subject;
				$subContent	=	str_replace($replacesub,$replace_with_sub,$subContent);
				$issuccess = $this->send_email_template($replaceav, $replace_withav, 'reset-password', $user->email,$subContent,$set->primary_email); 
				if($issuccess){
					return true;
				}else{
					return false;
		...
		public function agentsendResetEmail($email, $token)
		{
			$set = Admin::where('id',1)->first();
		//Retrieve the user from the database
		$user = DB::table('agents')->where('email', $email)->first();
		//Generate, the password reset link. The token generated is embedded in the link
		$link = \URL::to('/') . '/agents/password/reset/' . $token . '?email=' . urlencode($user->email);
...
public function AdminPhoneBookingTicket($id, $message, $pd = 0){
		$success = false;
		$mailmessage = $message;
		...
		if(@$fetchedData->farequoteob_log != ''){
			...
			if(isset($response->Response->Results->Segments[0])){
				...
				$flightname = $allflighdata[0]->Airline->AirlineName;
			}
		}
		if(MyConfig::where('meta_key','msg_status')->first()->meta_value == 'msgnine'){
			...
			// $smscontent = $message;
			// $mobileNumber = "91".$set->phone;
			// $senderId = "ZAPFLI";
			// $replacesub = array('{name}', '{source}', '{destination}', '{PNR}', '{BookingID}','{bookingid}', '{URL}', '{travel_date}', '{flight_name}');					
			// $replace_with_sub = array(@$name, @$fetchedData->source, @$fetchedData->destination, @$fetchedData->pnr, $set->ref_prefix.'-'.@$fetchedData->id,$set->ref_prefix.'-'.@$fetchedData->id, \URL::to('/'), date('d/m/Y', strtotime(@$fetchedData->depart_date)), $flightname);
			...
			// $route = "4";
			// $postData = array(
			// 'mobiles' => $mobileNumber.',919953534897',
			// 'message' => $message,
			// 'sender' => $senderId,
			// 'route' => $route
			// );
			...
			$replacesub = array('{name}', '{source}', '{destination}', '{PNR}', '{BookingID}','{bookingid}', '{URL}', '{travel_date}', '{flight_name}');					
			$replace_with_sub = array(@$name, @$fetchedData->source, @$fetchedData->destination, @$fetchedData->pnr, $set->ref_prefix.'-'.@$fetchedData->id, $set->ref_prefix.'-'.@$fetchedData->id, \URL::to('/'), date('d/m/Y', strtotime(@$fetchedData->depart_date)), $flightname);
			$subContent	=	str_replace($replacesub,$replace_with_sub,$mailmessage);
			if($pd == 1){
				//Mail::to($set->primary_email)->cc('mukesh@zapbooking.com')->send(new CommonMail($subContent, "Ticket Notification", $set->primary_email));
			}else{
				
				Mail::to($set->primary_email)->cc($set->primary_email)->send(new CommonMail($subContent, "Ticket Notification", $set->primary_email));
			}
		//echo json_encode(array('success'=>$success,'message'=>$message));
	}
	
	public static function fileupload($file = '', $fileid = '',$fieldname = '',$fieldname2 = ''){
		?>
