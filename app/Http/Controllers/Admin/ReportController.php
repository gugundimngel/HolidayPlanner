...
class ReportController extends Controller
{
	...
	if($request->type == 'b2b'){
		DB::enableQueryLog();
		$query 		= BookingDetail::where('type','=','b2b' )->where('status','=',1 )->with(['paymentdetail','agent']);
		...
		$query 		= BookingDetail::where('type','=','b2c')->where('status','=',1)->with(['paymentdetail']);
		if ($request->has('from')) 
		{
		...
		return view('Admin.report.dailysale',compact(['b2clists','b2blists'])); 	
	...
	{  
		$lists = array();
		if ($request->has('ref')) {
			$query = WalletHistory::where('id','!=','')->with(['user']);
	...
	//DB::enableQueryLog();
		...
		if(@$request->type == 'excel'){
			...
			print "\n";
				...
				return view('Admin.report.ledger',compact(['lists']));
			} 
			}else if(@$request->type == 'pdf'){
				if(@$totalData !== 0){
				$pdf = PDF::loadView('Admin.report.pdfledger', compact('lists'));
