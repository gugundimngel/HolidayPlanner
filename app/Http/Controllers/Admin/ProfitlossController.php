...
class ProfitlossController extends Controller
{
	...
	public function index(Request $request)
	{
		...
		//DB::enableQueryLog();
		$query 		= BookingDetail::where('status','=',1 )->with(['paymentdetail']);
		if(isset($_GET['type']) && $_GET['type'] == 'b2b'){
			$type = 'b2b';
			$query->where('type','=','b2b' );
		}else if(isset($_GET['type']) && $_GET['type'] == 'b2c'){
			$query->where('type','=','b2c' );
		}
		...
		if(isset($_GET['action']) && $_GET['action'] == 'print'){
			$pdf = PDF::loadView('Admin.profitloss.pdf', compact('lists'));
		...
		return view('Admin.profitloss.index',compact(['lists', 'totalData'])); 	
