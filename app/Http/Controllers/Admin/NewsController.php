...
class NewsController extends Controller
{
	...
	public function index(Request $request)
	{
		...
		return view('Admin.news.index',compact(['lists', 'totalData']));	
	}
	...
	public function store(Request $request)
	{
		...
		if ($request->isMethod('post')) 
		{
			...
			$obj->slug	=	$this->createlocSlug('blogs',@$requestData['title']);
			$saved				=	$obj->save();  
	...
	public function editnewsPage(Request $request, $id = NULL)
	{	
		//check authorization start	
			 $check = $this->checkAuthorizationAction('cmspages', $request->route()->getActionMethod(), Auth::user()->role);
			if($check)
		...
		if ($request->isMethod('post')) 
		{
			...
			$this->validate($request, [
										'title' => 'required|max:255|unique:blogs,title,'.@$requestData['id'],
			...
			$obj->image			=	@$topinclu_image;
			$obj->content		=	@$requestData['description'];
			$obj->slug	=	$this->createlocSlug('blogs',@$requestData['title'], $requestData['id']);
			...
			{
				return Redirect::to('/admin/news')->with('success', 'News '.Config::get('constants.edited'));
			}				
		}
		else
		{	
			if(isset($id) && !empty($id))
			{
				...
				if(Blog::where('id', '=', $id)->exists()) 
				{
					...
					return view('Admin.news.edit', compact(['fetchedData']));
				}
				else
				{
					return Redirect::to('/admin/news')->with('error', 'News '.Config::get('constants.not_exist'));
				}	
			}
			else
			{
				return Redirect::to('/admin/news')->with('error', Config::get('constants.unauthorized'));
			}		
		}				
	}
