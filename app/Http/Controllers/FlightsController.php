...
class FlightsController extends Controller
{
	...
	public function flightsG(Request $request) 
    {
			return view('Frontend.flightnew'); 
	}	
	...
	public function FareQuote(Request $request) 
    {
		...
		if(isset($_GET['IsReturn']) && $_GET['IsReturn'] != 'False'){
			...
			return view('Frontend.flights.farequote', compact(['resultdata','resultdataib','mytotal','expired'])); 
		}else{
			$expired = 1;
			$err = $resultdata->Response->Error->ErrorMessage.' '.$resultdataib->Response->Error->ErrorMessage;
			return view('Frontend.flights.farequote', compact(['expired','err'])); 
		}
		}else{
	 ...
	 public function TravelPlans(){
		...
		for($i = 0; $i<count($resu->pTrvCoverDtlsList_out); $i++){
...
// 		dd($auth);
		...
		//echo '<pre>'; print_r($flightresult); die;
			if(@$flightresult->Response->Error->ErrorCode == 0){
				...
				if($jt == 1){
					...
					return view('Frontend.flights.oneway', compact(['flightresult','calenderresult', 'is_international', 'calenderresult1'])); 
				}else if($jt == 3){
				...
				$calenderresult1 = array();
					if($originexplode[2] == 'India' && $desexplode[2] == 'India'){
						...
						//echo '<pre>'; print_r($flightresult); die;
					return view('Frontend.flights.multiway', compact(['flightresult','calenderresult', 'is_international', 'calenderresult1'])); 
				}else if($jt == 2){
					if($originexplode[2] == 'India' && $desexplode[2] == 'India'){
						$result = \Cache::get($flightresult->Response->TraceId);
						$flightresult = json_decode($result);
						return view('Frontend.flights.roundtrip', compact(['flightresult','source','destin','datearival','datedeparture'])); 
					}else{
						$calenderdata = '{
				"JourneyType": "'.$jt.'",
				"EndUserIp": "119.18.54.84",
			...
			$calendesrresult = $this->postcurlRequest($urlcsearch,$calenderdata); 
			$calenderresult = json_decode($calendesrresult);
						return view('Frontend.flights.international', compact(['flightresult','source','destin','datearival','datedeparture','calenderresult'])); 
					}
					
				}
			}else{
				
				return Redirect::to('/search-again')->with('error', Config::get('constants.unauthorized'));
			}
	}
	
	public function flightListnew(Request $request) 
    {
		$flghtapi = WebsiteSetting::where('id', '!=', '')->first();
		$auth = $this->GetAuthenticate();
		Session::put('allrequest',$request->all());
	...
	$log = ['description' => $jsonresponse];
		...
		//echo '<pre>'; print_r($flightresult); die;
			...
			$calendesrresult = $this->postcurlRequest($urlcsearch,$calenderdata); 
			$calenderresult = json_decode($calendesrresult);
						return view('Frontend.flights.newinternational', compact(['flightresult','source','destin','datearival','datedeparture','calenderresult'])); 
					}
...
//IB9 OB9 2985  1974
	public function CalenderFare(Request $request){
		$flghtapi = WebsiteSetting::where('id', '!=', '')->first();
		$auth = $this->GetAuthenticate();
		$calenderdata = '{
				"JourneyType": "1",
				...
				"Destination": "BOM",
				"FlightCabinClass": "2",
				"PreferredDepartureTime": "2020-07-19T00:00:00"
				}
				],
				"Sources": null
				}';
	...
	public function ticket(Request $request){
		...
		if (\Cache::has('farequoteob'.$pessager['hfTraceId'].$pessager['hfRIndex'])){
			...
			$user_id = Session::get('useridd');
				...
				\Cache::forget('farequoteob'.$pessager['hfTraceId'].$pessager['hfRIndex']);
				$error = 'Ticket not issued due to technical reasons to contact 7969224444';
				return Redirect::to('/booking/error')->with('error', $error);
		}
		...
		$result_data .=	  '"AddressLine1": "123 test",
			  "AddressLine2": "",
				...
				if($resultdata->Response->Results->IsGSTMandatory == true && $ics == 0){
					...
					"GSTCompanyContactNumber": "7969224444",
					"GSTCompanyName": "HP BOOKING PRIVATE LIMITED",
...
$inexp = explode('/',$pessager['infantdob'][$ics]);
	...
	$result_data .=	'],';
		...
		$result_data .='"EndUserIp": "119.18.54.84",
			"TokenId": "'.$auth->TokenId.'",
			...
			//first parameter passed to Monolog\Logger sets the logging channel name
				...
				if($bookdetail->Response->ResponseStatus != 1 && $bookdetail->Response->Error->ErrorCode !=0){
					...
					/*  if($pessager['IsIntr'] == 1 || $pessager['IsIntr'] == 'true'){
						 ...
						 return Redirect::to('/Review/Checkout?tid='.$pessager['hfTraceId'].'&RIndex='.$pessager['hfRIndex'].'&isINT='.$pessager['IsIntr'])->with('error', $bookdetail->Response->Error->ErrorMessage);
					 } */
					 \Cache::forget('farequoteob'.$pessager['hfTraceId'].$pessager['hfRIndex']);
					 \Cache::forget('ssrob'.$pessager['hfTraceId'].$pessager['hfRIndex']);
					  $error = 'Ticket not issued due to technical reasons to contact 7969224444';
					//return Redirect::to('/booking/error')->with('error', $bookdetail->Response->Error->ErrorMessage);
					return Redirect::to('/booking/error')->with('error', $error);
					exit;
				}
...
//Decode pdf content
				...
				$subject = $Flight_Name." Booking PNR ". $fetchedData->pnr.": ".$DATE." ".@$booking->Response->Response->FlightItinerary->Segments[0]->Origin->Airport->CityName."-".@$booking->Response->Response->FlightItinerary->Segments[$tr]->Destination->Airport->CityName." for ".@$pessager['adulttitle'][0]." ".@$pessager['adultfirstname'][0]." ".@$pessager['adultlastname'][0];
					 ...
					 Mail::to(@$pessager['email'])->send(new TicketMail($fetchedData, $subject, $set->primary_email, $array));
					  Mail::to($set->primary_email)->cc('booking@holidaychacha.com')->send(new TicketMail($fetchedData, $subject, $set->primary_email, $array));
					 unlink($array['file']);
					 if(@Auth::user()){
					 }else{
			...
			// echo json_encode(array('success' =>false,'message'=>$err));
			 ...
			 // echo json_encode(array('success' =>true,'message'=>$response));
				...
				/*SMS */
				$pd = 1;
				$message = "Dear Admin, '{name}' have successfully booked a flight ticket from {source} to {destination}. PNR is '{PNR}' and HP Booking Id is '{BookingID}'. {travel_date} and {flight_name} For more bookings please visit {URL}. Thank you.";
				$this->AdminPhoneBookingTicket($booking_id, $message,$pd);
					...
					Session::forget('to_date');
					 \Cache::forget('farequoteob'.$pessager['hfTraceId'].$pessager['hfRIndex']);
					 \Cache::forget('ssrob'.$pessager['hfTraceId'].$pessager['hfRIndex']);
					return Redirect::to('/booking/success/'.base64_encode(convert_uuencode(@$bookingdetail->id)));
				}else{
					 \Cache::forget('farequoteob'.$pessager['hfTraceId'].$pessager['hfRIndex']);
					 \Cache::forget('ssrob'.$pessager['hfTraceId'].$pessager['hfRIndex']);
					 $error = 'Ticket not issued due to technical reasons to contact 7969224444';
	...
	public function Returnticket(Request $request){
		...
		if($pessager['IsReturn'] == 1){
			...
			$user_id = Session::get('useridd');
				...
				Session::forget('from_date');
				Session::forget('to_date');
				\Cache::forget('farequoteib'.$pessager['hfTraceId'].$pessager['hfIBRIndex']);
				$error = 'Ticket not issued due to technical reasons to contact 7969224444';
				return Redirect::to('/booking/error')->with('error', $error);
			}
			$resultdataib = json_decode($resultib);
	...
	$result_dataib .=	'],';
		...
		$result_dataib .='"EndUserIp": "119.18.54.84",
			"TokenId": "'.$auth->TokenId.'",
			"TraceId": "'.$pessager['hfTraceId'].'"
			}'; 
...
$nonlcc = true;
	...
	if($resultdataib->Response->Results->IsLCC ==1){
		...
		if($bookdetail->Response->ResponseStatus != 1 && $bookdetail->Response->Error->ErrorCode !=0){
			...
			//if($pessager['IsReturn'] == 1){
				$log = ['email' => @$pessager['email'],
        'obrequest' => $result_data, 'obreponse'=>$result, 'ibrequest'=>$result_dataib, 'ibrewsponse'=>$ssrdataib];
			...
			// echo json_encode(array('success' =>false,'message'=>$err));
			 ...
			 // echo json_encode(array('success' =>true,'message'=>$response));
				...
				/*SMS */
					...
					\Cache::forget( 'farequoteob'.$pessager['hfTraceId'].$pessager['hfRIndex']);
					\Cache::forget('farequoteib'.$pessager['hfTraceId'].$pessager['hfIBRIndex']);
					\Cache::forget( 'ssrdataib'.$pessager['hfTraceId'].$pessager['hfIBRIndex']);
					return Redirect::to('/booking/success/'.base64_encode(convert_uuencode(@$bookingdetail->id)));
				}else{
					\Cache::forget( 'ssrob'.$pessager['hfTraceId'].$pessager['hfRIndex']);
					\Cache::forget( 'farequoteob'.$pessager['hfTraceId'].$pessager['hfRIndex']);
		...
		if(isset($id) && !empty($id)) 
			{
				...
				if(BookingDetail::where('id', '=', $id)->exists()) 
				{
					$fetchedData = BookingDetail::where('id',$id)->with(['user'])->first();
					 return view('Frontend.flights.ticket', compact(['fetchedData']));
				}
				else
				{
		...
		if(isset($id) && !empty($id)) 
			{
				...
				{
					$fetchedData = BookingDetail::where('id',$id)->with(['user'])->first();
					 return view('Frontend.flights.booking-confirm', compact(['fetchedData']));
				}
				else
				{
					//return Redirect::to('/admin/contact')->with('error', 'Contact Not Exist');
